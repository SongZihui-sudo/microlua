...
static int tpanic( lua_State* L )
{
    ...
    return ( badexit( "PANIC: unprotected error in call to Lua API (%s)\n", msg, NULL ), 0 ); /* do not return to Lua */
}
...
= { 0, 0UL, 0UL, 0UL, 0UL, ( ~0UL ), { 0UL, 0UL, 0UL, 0UL, 0UL, 0UL, 0UL, 0UL, 0UL } };
...
void* debug_realloc( void* ud, void* b, size_t oldsize, size_t size )
{
    ...
    if ( mc->countlimit != ~0UL && size != oldsize )
    { /* count limit in use? */
        if ( mc->countlimit == 0 )
...
*/
static void checkobject( global_State* g, GCObject* o, int maybedead, int listage )
{
    if ( isdead( g, o ) )
        assert( maybedead );
    else
...
static lu_mem
checklist( global_State* g, int maybedead, int tof, GCObject* newl, GCObject* survival, GCObject* old, GCObject* reallyold )
{
    GCObject* o;
...
int lua_checkmemory( lua_State* L )
{
    ...
    totalshould = checklist( g, maybedead, 0, g->allgc, g->survival, g->old1, g->reallyold );

    /* check 'finobj' list */
    totalshould += checklist( g, 0, 1, g->finobj, g->finobjsur, g->finobjold1, g->finobjrold );
...
static int gc_age( lua_State* L )
{
    ...
    else
    {
        ...
        = { "new", "survival", "old0", "old1", "old", "touched1", "touched2" };
        GCObject* obj = gcvalue( o );
...
static int loadlib( lua_State* L )
{
    ...
    = { { LUA_GNAME, luaopen_base },  { "coroutine", luaopen_coroutine },
        { "debug", luaopen_debug },   { "io", luaopen_io },
        { "os", luaopen_os },         { "math", luaopen_math },
        { "string", luaopen_string }, { "table", luaopen_table },
        { "T", luaB_opentests },      { NULL, NULL } };
...
static const char* const statcodes[]
= { "OK", "YIELD", "ERRRUN", "ERRSYNTAX", MEMERRMSG, "ERRGCMM", "ERRERR" };
...
static int runC( lua_State* L, lua_State* L1, const char* pc )
{
    ...
    for ( ;; )
    {
        ...
        else if EQ ( "compare" )
        {
            ...
            int op = ( opt[0] == 'E' ) ? LUA_OPEQ : ( opt[1] == 'T' ) ? LUA_OPLT : LUA_OPLE;
            int a  = getindex;
            int b  = getindex;
