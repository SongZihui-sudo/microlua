...
static int lfs_hal_read( lfs_block_t block, lfs_off_t off, void* buffer, lfs_size_t size );
static int lfs_hal_prog( lfs_block_t block, lfs_off_t off, const void* buffer, lfs_size_t size );
static int lfs_hal_erase( lfs_block_t block );
...
int _lfs_rewind( int file ) { return lfs_file_rewind( ( lfs_file_t* )file ); }

int _lfs_unmount( void ) { return lfs_unmount(); }

int _lfs_remove( const char* path ) { return lfs_remove( path ); }
...
{
    return lfs_file_truncate( ( lfs_file_t* )file, size );
}

lfs_soff_t _lfs_tell( int file ) { return lfs_file_tell( ( lfs_file_t* )file ); }

int _lfs_stat( const char* path, struct lfs_info* info ) { return lfs_stat( path, info ); }
...
{
    return lfs_removeattr( path, type );
}
...
int _lfs_fflush( int file ) { return lfs_file_sync( ( lfs_file_t* )file ); }

lfs_soff_t _lfs_size( int file ) { return lfs_file_size( ( lfs_file_t* )file ); }

int _lfs_mkdir( const char* path ) { return lfs_mkdir( path ); }

int _lfs_dir_open( const char* path )
{
...
{
    return lfs_dir_read( ( lfs_dir_t* )dir, info );
}

int _lfs_dir_seek( int dir, lfs_off_t off )
{
    return lfs_dir_seek( ( lfs_dir_t* )dir, off );
}

lfs_soff_t _lfs_dir_tell( int dir ) { return lfs_dir_tell( ( lfs_dir_t* )dir ); }

int _lfs_dir_rewind( int dir ) { return lfs_dir_rewind( ( lfs_dir_t* )dir ); }
...
int _lfs_getc( int f )
{
    ...
    char buffer;
    int err            = _lfs_read( f, &buffer, sizeof( char ) );
    const char* errmsg = _lfs_errmsg( err );
...
int _lfs_ungetc( char c, int f )
{
    lfs_file_t* fp     = ( lfs_file_t* )f;
    int err            = _lfs_write( f, &c, sizeof( char ) );
    const char* errmsg = _lfs_errmsg( err );
    if ( !strcmp( errmsg, "Unknown error" ) )
    {
        return 1;
