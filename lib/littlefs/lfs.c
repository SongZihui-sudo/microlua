...
static int lfs_bd_read( const lfs_cache_t* pcache,
                        lfs_cache_t* rcache,
                        lfs_size_t hint,
                        lfs_block_t block,
                        lfs_off_t off,
...
static int lfs_bd_cmp( const lfs_cache_t* pcache,
                       lfs_cache_t* rcache,
                       lfs_size_t hint,
                       lfs_block_t block,
                       lfs_off_t off,
...
static int lfs_bd_flush( lfs_cache_t* pcache, lfs_cache_t* rcache, bool validate )
{
    if ( pcache->block != LFS_BLOCK_NULL && pcache->block != LFS_BLOCK_INLINE )
    {
        ...
        if ( validate )
        {
            ...
            int res = lfs_bd_cmp( NULL, rcache, diff, pcache->block, pcache->off, pcache->buffer, diff );
            if ( res < 0 )
            {
                return res;
            }
...
#ifndef LFS_READONLY
static int lfs_bd_prog( lfs_cache_t* pcache,
                        lfs_cache_t* rcache,
                        bool validate,
                        lfs_block_t block,
                        lfs_off_t off,
...
static int lfs_dir_compact( lfs_mdir_t* dir,
...
static int lfs_fs_rawtraverse( int ( *cb )( void* data, lfs_block_t block ), void* data, bool includeorphans );
...
static lfs_stag_t
lfs_dir_getslice( const lfs_mdir_t* dir, lfs_tag_t gmask, lfs_tag_t gtag, lfs_off_t goff, void* gbuffer, lfs_size_t gsize )
{
    ...
    while ( off >= sizeof( lfs_tag_t ) + lfs_tag_dsize( ntag ) )
    {
        ...
        if ( ( gmask & tag ) == ( gmask & ( gtag - gdiff ) ) )
        {
            ...
            err = lfs_bd_read( NULL, &lfs.rcache, diff, dir->pair[0], off + sizeof( tag ) + goff, gbuffer, diff );
            if ( err )
            {
                return err;
            }
...
static lfs_stag_t lfs_dir_get( const lfs_mdir_t* dir, lfs_tag_t gmask, lfs_tag_t gtag, void* buffer )
{
    return lfs_dir_getslice( dir, gmask, gtag, 0, buffer, lfs_tag_size( gtag ) );
}

static int lfs_dir_getread( const lfs_mdir_t* dir,
...
#ifndef LFS_READONLY
static int lfs_dir_traverse( const lfs_mdir_t* dir,
                             lfs_off_t off,
                             lfs_tag_t ptag,
                             const struct lfs_mattr* attrs,
                             int attrcount,
                             ...
                             void* data )
{
    ...
    while ( true )
    {
        ...
        if ( lfs_tag_id( tmask ) != 0 )
        {
            ...
            int filter
            = lfs_dir_traverse( dir, off, ptag, attrs, attrcount, 0, 0, 0, 0, 0, lfs_dir_traverse_filter, &tag );
            if ( filter < 0 )
            {
                return filter;
            }
        ...
        else if ( lfs_tag_type3( tag ) == LFS_FROM_MOVE )
        {
            ...
            int err         = lfs_dir_traverse( buffer,
                                        0,
                                        0xffffffff,
                                        NULL,
                                        0,
...
static lfs_stag_t lfs_dir_fetchmatch( lfs_mdir_t* dir,
                                      ...
                                      void* data )
{
    ...
    for ( int i = 0; i < 2; i++ )
    {
        ...
        while ( true )
        {
            ...
            off += lfs_tag_dsize( ptag );
            int err
            = lfs_bd_read( NULL, &lfs.rcache, lfs.cfg->block_size, dir->pair[0], off, &tag, sizeof( tag ) );
            if ( err )
            {
            ...
            {
                uint8_t dat;
                err = lfs_bd_read( NULL, &lfs.rcache, lfs.cfg->block_size, dir->pair[0], off + j, &dat, 1 );
                if ( err )
                {
                    if ( err == LFS_ERR_CORRUPT )
                    {
            ...
            else if ( lfs_tag_type1( tag ) == LFS_TYPE_TAIL )
            {
                ...
                err = lfs_bd_read(
                NULL, &lfs.rcache, lfs.cfg->block_size, dir->pair[0], off + sizeof( tag ), &temptail, 8 );
                if ( err )
                {
                    if ( err == LFS_ERR_CORRUPT )
                    {
...
static int lfs_dir_getgstate( const lfs_mdir_t* dir, lfs_gstate_t* gstate )
{
    lfs_gstate_t temp;
    lfs_stag_t res
    = lfs_dir_get( dir, LFS_MKTAG( 0x7ff, 0, 0 ), LFS_MKTAG( LFS_TYPE_MOVESTATE, 0, sizeof( temp ) ), &temp );
...
static int lfs_dir_getinfo( lfs_mdir_t* dir, uint16_t id, struct lfs_info* info )
{
    ...
    lfs_stag_t tag = lfs_dir_get( dir,
    ...
    struct lfs_ctz ctz;
    tag
    = lfs_dir_get( dir, LFS_MKTAG( 0x700, 0x3ff, 0 ), LFS_MKTAG( LFS_TYPE_STRUCT, id, sizeof( ctz ) ), &ctz );
    if ( tag < 0 )
    {
        return ( int )tag;
    }
...
static lfs_stag_t lfs_dir_find( lfs_mdir_t* dir, const char** path, uint16_t* id )
{
    ...
    nextname:
        ...
        *path = name;
        ...
        {
            lfs_stag_t res = lfs_dir_get( dir,
                                          LFS_MKTAG( 0x700, 0x3ff, 0 ),
...
{
    int err
    = lfs_bd_prog( &lfs.pcache, &lfs.rcache, false, commit->block, commit->off, ( const uint8_t* )buffer, size );
    if ( err )
    {
        return err;
    }
...
static int lfs_dir_commitattr( struct lfs_commit* commit, lfs_tag_t tag, const void* buffer )
{
    ...
    else
    {
        ...
        for ( lfs_off_t i = 0; i < dsize - sizeof( tag ); i++ )
        {
            ...
            uint8_t dat;
            err
            = lfs_bd_read( NULL, &lfs.rcache, dsize - sizeof( tag ) - i, disk->block, disk->off + i, &dat, 1 );
            if ( err )
            {
                return err;
            }
...
static int lfs_dir_commitcrc( struct lfs_commit* commit )
{
    ...
    while ( commit->off < end )
    {
        ...
        err = lfs_bd_prog( &lfs.pcache, &lfs.rcache, false, commit->block, commit->off, &footer, sizeof( footer ) );
        if ( err )
        {
            return err;
        }
    ...
    while ( off < end )
    {
        ...
        for ( lfs_off_t i = off; i < noff + sizeof( uint32_t ); i++ )
        {
            ...
            uint8_t dat;
            err = lfs_bd_read( NULL, &lfs.rcache, noff + sizeof( uint32_t ) - i, commit->block, i, &dat, 1 );
            if ( err )
            {
                return err;
            }
...
static int lfs_dir_alloc( lfs_mdir_t* dir )
{
    ...
    int err = lfs_bd_read(
    NULL, &lfs.rcache, sizeof( dir->rev ), dir->pair[0], 0, &dir->rev, sizeof( dir->rev ) );
    dir->rev = lfs_fromle32( dir->rev );
    if ( err && err != LFS_ERR_CORRUPT )
    {
        return err;
...
static int lfs_dir_drop( lfs_mdir_t* dir, lfs_mdir_t* tail )
{
    ...
    err = lfs_dir_commit( dir,
                          LFS_MKATTRS( { LFS_MKTAG( LFS_TYPE_TAIL + tail->split, 0x3ff, 8 ),
                                         tail->tail } ) );
...
#ifndef LFS_READONLY
static int
lfs_dir_split( lfs_mdir_t* dir, const struct lfs_mattr* attrs, int attrcount, lfs_mdir_t* source, uint16_t split, uint16_t end )
{
                            ...
                            uint16_t end )
{
    ...
    while ( end - begin > 1 )
    {
        // find size
        lfs_size_t size = 0;
        int err         = lfs_dir_traverse( source,
                                    0,
                                    0xffffffff,
                                    attrs,
                                    attrcount,
...
static int lfs_dir_commit( lfs_mdir_t* dir, const struct lfs_mattr* attrs, int attrcount )
{
    ...
    if ( dir->erased || dir->count >= 0xff )
    {
        ...
        dir, dir->off, dir->etag, attrs, attrcount, 0, 0, 0, 0, 0, lfs_dir_commit_commit, &( struct lfs_dir_commit_commit ){ &commit } );
        lfs_pair_fromle32( dir->tail );
        if ( err )
        {
...
static int lfs_rawmkdir( const char* path )
{
    ...
    err
    = lfs_dir_commit( &dir, LFS_MKATTRS( { LFS_MKTAG( LFS_TYPE_SOFTTAIL, 0x3ff, 8 ), pred.tail } ) );
...
static int lfs_dir_rawopen( lfs_dir_t* dir, const char* path )
{
    ...
    else
    {
        ...
        lfs_stag_t res = lfs_dir_get( &dir->m,
                                      LFS_MKTAG( 0x700, 0x3ff, 0 ),
                                      LFS_MKTAG( LFS_TYPE_STRUCT, lfs_tag_id( tag ), 8 ),
                                      pair );
        if ( res < 0 )
...
static int lfs_ctz_find( const lfs_cache_t* pcache,
                         lfs_cache_t* rcache,
                         lfs_block_t head,
                         lfs_size_t size,
                         lfs_size_t pos,
...
#ifndef LFS_READONLY
static int lfs_ctz_extend( lfs_cache_t* pcache,
                           lfs_cache_t* rcache,
                           lfs_block_t head,
                           lfs_size_t size,
                           lfs_block_t* block,
    ...
    while ( true )
    {
...

        {
            ...
            for ( lfs_off_t i = 0; i < skips; i++ )
            {
                ...
                if ( i != skips - 1 )
                {
                    err = lfs_bd_read( NULL, rcache, sizeof( nhead ), nhead, 4 * i, &nhead, sizeof( nhead ) );
                    nhead = lfs_fromle32( nhead );
                    if ( err )
                    {
                        return err;
...
static int lfs_ctz_traverse( const lfs_cache_t* pcache,
                             lfs_cache_t* rcache,
                             lfs_block_t head,
                             lfs_size_t size,
                             ...
                             void* data )
{
    ...
    while ( true )
    {
        ...
        lfs_block_t heads[2];
        int count = 2 - ( index & 1 );
        err = lfs_bd_read( pcache, rcache, count * sizeof( head ), head, 0, &heads, count * sizeof( head ) );
...
static int lfs_file_rawopencfg( lfs_file_t* file, const char* path, int flags, const struct lfs_file_config* file_cfg )
{
#ifndef LFS_READONLY
    // deorphan if we haven't yet, needed at most once after poweron
    ...
    for ( unsigned i = 0; i < file->file_cfg->attr_count; i++ )
    {
        ...
        if ( ( file->flags & LFS_O_RDONLY ) == LFS_O_RDONLY )
        {
            ...
            = lfs_dir_get( &file->m,
                           ...
                           LFS_MKTAG( LFS_TYPE_USERATTR + file->file_cfg->attrs[i].type,
                                      file->id,
...
static int lfs_file_relocate( lfs_file_t* file )
{
    while ( true )
    {
        ...
        for ( lfs_off_t i = 0; i < file->off; i++ )
        {
            ...
            {
                err = lfs_bd_read( &file->cache, &lfs.rcache, file->off - i, file->block, i, &data, 1 );
                if ( err )
                {
                    return err;
                }
...
static int lfs_file_rawsync( lfs_file_t* file )
{
    ...
    if ( ( file->flags & LFS_F_DIRTY ) && !lfs_pair_isnull( file->m.pair ) )
    {
        ...
        LFS_MKATTRS( { LFS_MKTAG( type, file->id, size ), buffer },
                     { LFS_MKTAG( LFS_FROM_USERATTRS, file->id, file->file_cfg->attr_count ),
                       file->file_cfg->attrs } ) );
        if ( err )
        {
            file->flags |= LFS_F_ERRED;
...
static lfs_ssize_t lfs_file_rawread( lfs_file_t* file, void* buffer, lfs_size_t size )
{
    ...
    while ( nsize > 0 )
    {
        ...
        if ( file->flags & LFS_F_INLINE )
        {
            int err = lfs_dir_getread( &file->m,
                                       NULL,
                                       &file->cache,
                                       lfs.cfg->block_size,
                                       LFS_MKTAG( 0xfff, 0x1ff, 0 ),
        ...
        {
            int err
            = lfs_bd_read( NULL, &file->cache, lfs.cfg->block_size, file->block, file->off, data, diff );
            if ( err )
            {
                return err;
            }
    ...
    while ( nsize > 0 )
    {
        ...
        if ( !( file->flags & LFS_F_WRITING ) || file->off == lfs.cfg->block_size )
        {
            if ( !( file->flags & LFS_F_INLINE ) )
            {
                ...
                lfs_alloc_ack();
                int err = lfs_ctz_extend(
                &file->cache, &lfs.rcache, file->block, file->pos, &file->block, &file->off );
                if ( err )
                {
                    file->flags |= LFS_F_ERRED;
                    return err;
        ...
        while ( true )
        {
            int err = lfs_bd_prog( &file->cache, &lfs.rcache, true, file->block, file->off, data, diff );
            if ( err )
            {
                if ( err == LFS_ERR_CORRUPT )
                {
...
static int lfs_file_rawtruncate( lfs_file_t* file, lfs_off_t size )
{
    ...
    if ( size < oldsize )
    {
        ...
        err = lfs_ctz_find(
        NULL, &file->cache, file->ctz.head, file->ctz.size, size, &file->block, &file->off );
        if ( err )
        {
            return err;
        }
...
static int lfs_rawremove( const char* path )
{
    ...
    if ( lfs_tag_type3( tag ) == LFS_TYPE_DIR )
    {
        ...
        lfs_block_t pair[2];
        lfs_stag_t res = lfs_dir_get( &cwd,
                                      LFS_MKTAG( 0x700, 0x3ff, 0 ),
                                      LFS_MKTAG( LFS_TYPE_STRUCT, lfs_tag_id( tag ), 8 ),
                                      pair );
        if ( res < 0 )
...
static int lfs_rawrename( const char* oldpath, const char* newpath )
{
    ...
    else if ( lfs_tag_type3( prevtag ) == LFS_TYPE_DIR )
    {
        ...
        lfs_stag_t res
        = lfs_dir_get( &newcwd, LFS_MKTAG( 0x700, 0x3ff, 0 ), LFS_MKTAG( LFS_TYPE_STRUCT, newid, 8 ), prevpair );
        if ( res < 0 )
        {
            return ( int )res;
        }
...
static int lfs_commitattr( const char* path, uint8_t type, const void* buffer, lfs_size_t size )
{
    ...
    return lfs_dir_commit( &cwd, LFS_MKATTRS( { LFS_MKTAG( LFS_TYPE_USERATTR + type, id, size ), buffer } ) );
}
#endif
...
static int lfs_rawmount( const struct lfs_config* cfg )
{
    ...
    while ( !lfs_pair_isnull( dir.tail ) )
    {
        ...
        lfs_stag_t tag = lfs_dir_fetchmatch( &dir,
                                             dir.tail,
                                             LFS_MKTAG( 0x7ff, 0x3ff, 0 ),
                                             LFS_MKTAG( LFS_TYPE_SUPERBLOCK, 0, 8 ),
                                             NULL,
                                             lfs_dir_find_match,
                                             &( struct lfs_dir_find_match ){ "littlefs", 8 } );
        if ( tag < 0 )
        {
            err = tag;
            goto cleanup;
...
static int lfs_fs_parent_match( void* data, lfs_tag_t tag, const void* buffer )
{
    ...
    lfs_block_t child[2];
    int err
    = lfs_bd_read( &lfs.pcache, &lfs.rcache, lfs.cfg->block_size, disk->block, disk->off, &child, sizeof( child ) );
    if ( err )
    {
        return err;
    }
...
static lfs_stag_t lfs_fs_parent( const lfs_block_t pair[2], lfs_mdir_t* parent )
{
    ...
    while ( !lfs_pair_isnull( parent->tail ) )
    {
        ...
        lfs_stag_t tag
        = lfs_dir_fetchmatch( parent,
                              parent->tail,
                              LFS_MKTAG( 0x7ff, 0, 0x3ff ),
                              LFS_MKTAG( LFS_TYPE_DIRSTRUCT, 0, 8 ),
                              NULL,
                              lfs_fs_parent_match,
                              &( struct lfs_fs_parent_match ){ { pair[0], pair[1] } } );
...
int lfs_file_opencfg( lfs_file_t* file, const char* path, int flags, const struct lfs_file_config* cfg )
{
    int err = LFS_LOCK;
    if ( err )
    {
